...
public class MergerProperties extends PepperModuleProperties {
	...
	public static final String PROP_APPEND_TEXT = "appendText";
	public static final String PROP_DOC_MAPPING_PREFIX = "docMapping.";

	public MergerProperties() {
		this.addProperty(new PepperModuleProperty<>(PROP_PUNCTUATIONS, String.class, "Determines the punctuation characters used to be ignored for detecting equal textual data. The value is a comma separated list, each entry must be surrounded by a quot: 'PUNCTUATION' (, 'PUNCTUATION')* .", PUNCTUATION_DEFAULT));
		this.addProperty(new PepperModuleProperty<>(PROP_ESCAPE_MAPPING, String.class, "Determines the mapping used in normalization step, to map special characters like umlauts. This value is a comma separated list of mappings: \"REPLACED_CHARACTER\" : \"REPLACEMENT\" (, \"REPLACED_CHARACTER\" : \"REPLACEMENT\")*", ESCAPE_MAPPING_DEFAULT));
		...
		this.addProperty(new PepperModuleProperty<>(PROP_COPY_NODES, Boolean.class, "Determines if SSpan and SStructure nodes should be copied or merged. Merged means to move all annotations to the equivalent in base document. If value is true they will be copied.", false, false));
		this.addProperty(new PepperModuleProperty<>(PROP_APPEND_TEXT, Boolean.class, "Instead of merging the documents their are appended.", false, false));
	}
	...
	public boolean isAppendText() {
		return (Boolean) getProperty(PROP_APPEND_TEXT).getValue();
	}
}
